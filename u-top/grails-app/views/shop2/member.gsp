<!doctype html>
<%@page import="com.surelution.utop.DeliveryTicket.DeliveryStatus"%>
  ...
  <div id="TabbedPanels1" class="TabbedPanels">
	  ...
	  <div class="TabbedPanelsContentGroup">
	    <div class="TabbedPanelsContent" style="padding-top:20px;">
			...
			<g:set var="ticket" value="${DeliveryTicket.findBySaleOrder(order) }"/>
			<div class="car_box" style="width:90%; margin:0 auto;">
			  <ul style="border-top:none;">
			    <h2 style="text-align:left;"<g:if test="${ticket?.status == DeliveryStatus.READY }"> onclick="showHiddenQr(box,'${createLink(controller:'shop2', action:'deliveryQr', id:ticket?.id)}')"</g:if>>提货号：<g:if test="${ticket }">${ticket.sn }</g:if><g:else>尚无</g:else><span>${order.status }</span></h2>
